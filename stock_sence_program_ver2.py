...
rounded_style = """
    <style>
        .rounded-rectangle {
            ...
            background-color: #910a1e; /* Background color */
            text-align: center; /* Center text alignment */
            font-size: 20px; /* Font size */
            color: #f2e4e6; /* Font color - change this value to your desired color */
        }
    </style>
"""
...
filtered_df = filtered_df.groupby(['DLP', 'DLPC', 'store', 'color']).agg({'total_quantity': 'sum', 'total_inventory': 'max', 'avg_purchase_price': 'avg'}).reset_index()
